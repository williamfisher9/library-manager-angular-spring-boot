...
export class ItemDetailsComponent implements OnInit {
    ...
    this.appService
      ...
      .subscribe((res) => {
        ...
        this.route.queryParamMap.subscribe(
          (params) => (this.itemId = params.get('itemId'))
        );

        this.appService
          .getItemById(this.itemId, this.username, this.password)
          .subscribe((res) => {
  ...
  setAsWatched() {
    this.appService.setItemAsWatched(this.userId, this.username, this.password, !this.watched, this.itemId).subscribe(res => {
      if(res.status == 200){
        this.watched = res.response.watched;
      }
    })
  }
}
